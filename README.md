...
npm run seed              # starter exercises, a demo admin, and a sample workout
npm run dev
```

Open the URL printed by the dev server and sign in as `admin@example.com` / `changeme123`.

> [!IMPORTANT]
> `SESSION_SECRET` has no default. There is deliberately no fallback baked into
> the repo — one would let anyone holding a copy mint an admin session.
...
`SESSION_SECRET` is generated by Render on first deploy (`generateValue` in the Blueprint). Set it by hand only if you are configuring a service without the Blueprint — any 32+ character random string works, and changing it signs every account out.

Create the first admin against Atlas — there is no public sign-up, so without this nobody can sign in:

```bash
npx tsx --env-file=.env.atlas.local scripts/create-user.ts \
  --email you@example.com --name 'Your Name' --role admin
```

It only inserts, so unlike the seed it needs no `--force`. Every other account is then created from **People** inside the app.

Seed Atlas instead if you want the starter exercise library and do not mind losing what is there:

```bash
npx tsx --env-file=.env.atlas.local scripts/seed.ts --force
```

`--force` is required for any non-local URI. Seeding deletes every exercise, workout **and account** first, so the guard exists to stop a stray `npm run seed` from wiping real history.

Workouts logged before accounts existed have no owner and are invisible to every account. Assign them once:

```bash
npx tsx --env-file=.env.atlas.local scripts/backfill-workout-owner.ts --email you@example.com
```

It only touches workouts that still have no owner, so it is safe to re-run.
